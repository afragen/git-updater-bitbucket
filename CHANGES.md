...
* security: contribute `bitbucket.org`/`api.bitbucket.org` and configured Bitbucket Server hosts through the new `gu_credential_hosts` filter so tokens are only sent to authorized hosts, and reject a `remote_install()` download link on an unauthorized host (requires Git Updater 14.4+)
